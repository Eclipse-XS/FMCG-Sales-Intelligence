"""Import the saved Draw.io XML with mxGraph and render both ER diagrams locally.
...
def main():
    ...
    sources = [BASE / (name + '_erd_readable_v2.drawio') for name in ['fmcg_logical','fmcg_physical']]
    pages = [ET.parse(source).find('diagram') for source in sources]
    assert len(pages) == 2
    checks = []
    with tempfile.TemporaryDirectory(prefix='fmcg-erd-check-') as temp:
        work = Path(temp)
        for source, page in zip(sources, pages):
            ...
            ET.ElementTree(svg).write(BASE / f'{slug}.svg', encoding='utf-8', xml_declaration=True)
    ...
    report_path = BASE / 'fmcg_layout_render_validation.json'
    report = {}
    report.update(mxgraph_import='PASS', mxgraph_version='4.2.2', native_pages=checks,
                  ...
                  drawio_sha256={source.name:hashlib.sha256(source.read_bytes()).hexdigest() for source in sources})

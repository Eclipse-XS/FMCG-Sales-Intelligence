...
CONFIG=ROOT/"config/modeling"

def read(name):
    return yaml.safe_load((CONFIG/f"{name}.yaml").read_text(encoding="utf-8"))

def dataset(name):
    return pl.read_parquet(ROOT / "data" / "processed" / name / f"{name}_v1.parquet")
...
def test_forecasting_temporal_splits_are_ordered_purged_and_match_counts():
    ...
    for name in ("selection_train","validation","final_train","test"):
        block=s[name]
        actual=f.filter((pl.col("prediction_date")>=as_date(block["start"]))&(pl.col("prediction_date")<=as_date(block["end"]))&pl.col("target_observed_next_7d")).height
        assert actual==block["rows"]

def test_stockout_contract_excludes_future_outcomes_and_split_has_events():
    c=read("stockout_classification")
    f=dataset("stockout")
    allowed={x for values in c["features"].values() for x in values}
    ...
    for name in ("selection_train","validation","final_train","test"):
        block=c["split"][name]
        x=f.filter((pl.col("prediction_date")>=as_date(block["start"]))&(pl.col("prediction_date")<=as_date(block["end"]))&(pl.col("event_observed")|pl.col("horizon_complete")))
...
def test_segmentation_uses_one_independent_latest_snapshot():
    ...
    latest=as_date(c["primary_snapshot"])

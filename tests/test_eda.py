...
def test_generator_preserves_complete_demand_but_sales_remain_positive_only():
    source = (ROOT / "src/generation/generate_dev_data.py").read_text(encoding="utf-8")
...
def test_eda_manifest_and_all_section_metrics_exist():
    manifest = json.loads((ROOT / "reports/eda/run_manifest.json").read_text(encoding="utf-8"))
    assert manifest["status"] == "PASS"
    for section in manifest["sections"]:
        assert (ROOT / f"reports/eda/{section}/metrics.json").exists()

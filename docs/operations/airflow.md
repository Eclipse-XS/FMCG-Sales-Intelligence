...
`platform/airflow/dags/fmcg_platform.py` defines a DAG with the dependency chain `source check → extract → dbt build → build datasets → quality`. The project root is resolved from `FSI_PROJECT_ROOT`, with the repository containing the DAG as the local default; no machine-specific absolute path is embedded.

...
Runtime status: **configured, not executed**. No project ID or service-account credentials were supplied. `platform/dbt/profiles.yml` includes a BigQuery target and `platform/airbyte/postgres_to_bigquery.json` defines the intended raw landing streams.

...
Kaplan–Meier is the descriptive cohort baseline. Predictive V1 uses an L2-penalized Cox proportional hazards model selected from the predeclared penalizers in `config/modeling/stockout_survival.yaml` by validation Harrell C-index. Higher model partial hazard means shorter expected event-free time; the C-index implementation therefore negates risk for lifelines' predicted-time orientation.
...
Run with `python -m fmcg_sales_intelligence.science.cli run --task stockout_survival`. The stable canonical output is DVC-managed at `artifacts/canonical/stockout_survival_v1/`. No DVC remote is configured.

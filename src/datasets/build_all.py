...
ROOT=Path(__file__).resolve().parents[2];DB=ROOT/"data/warehouse/fmcg.duckdb";OUT=ROOT/"data/processed";REPORTS=ROOT/"reports/data_engineering"
...
def main():
    ...
    with duckdb.connect(str(DB),read_only=True) as c:
        ...
        promotion=query(c,"""select p.*,d.promotion_type,d.discount_type,d.discount_value,s.region_id,s.store_type,s.channel from analytics.mart_promotion_performance p join analytics.dim_promotion d using(promotion_id) join analytics.dim_store s using(store_id)""")
